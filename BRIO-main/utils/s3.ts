...
// Generates a temporary, signed URL to READ a private S3 object. Needed because
// the bucket blocks public access, so the raw object URL returns 403. Used to
// display uploaded files (PDF viewer, image, office viewers) in the browser.
export async function getSignedFileUrl(file_key: string): Promise<string> {
  const s3 = new S3Client({
    region: process.env.AWS_S3_REGION!,
    credentials: {
      accessKeyId: process.env.AWS_S3_ACCESS_KEY_ID!,
