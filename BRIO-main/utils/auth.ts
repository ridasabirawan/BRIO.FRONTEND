...
const authConfig: NextAuthConfig = {
  ...
  callbacks: {
    authorized({ auth, request: { nextUrl } }) {
      ...
      // Enforce bans: a banned user can stay signed in but is locked out of all
      // protected areas and sent to the /banned page.
      if (
        isLoggedIn &&
        (auth!.user as { banned?: boolean }).banned &&
        isProtected &&
        nextUrl.pathname !== "/banned"
      ) {
        return Response.redirect(new URL("/banned", nextUrl.origin));
      }
