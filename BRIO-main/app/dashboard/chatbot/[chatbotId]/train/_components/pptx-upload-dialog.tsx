...
export default function PPTXUploadDialog() {
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    ...
    if (selectedFile && selectedFile.type === PPTX_MIME) {
  ...
  const handleSubmit = async () => {
    if (!file) return;
    try {
      await uploadFile(file, "pptx");
      toast.success("Your PPTX is being processed! Please don't close the app.");
      setFile(null);
      setIsOpen(false);
      reset();
      ...
      console.error(error);
      toast.error("Error adding PPTX. Please try again.");
      reset();
    }
  };
          ...
          >
            ...
            {file ? (
              <div className="flex items-center justify-center gap-2">
                <Projector className="w-6 h-6 text-foreground" strokeWidth={0.8} />
              ...
              <div>
                <Projector className="w-6 h-6 mx-auto mb-2 text-foreground" />
