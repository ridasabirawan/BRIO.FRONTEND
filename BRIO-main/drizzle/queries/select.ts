...
import { sql, count, eq, and, or, ilike, asc } from "drizzle-orm";
import {
  chatbots,
  conversations,
  kbSources,
...
export async function getKbSourcesForChatPage(chatbotId: string) {
  ...
  try {
    ...
    // Files live in a private S3 bucket, so the stored public URL returns 403.
    // Swap in a short-lived signed URL for anything stored in S3 (not url/text).
    const S3_TYPES = new Set([
      "pdf",
      "docx",
      "doc",
      "txt",
    ...
    const sources = await Promise.all(
      rows.map(async ({ sourceKey, ...s }) => {
        if (S3_TYPES.has(s.type) && sourceKey) {
          try {
            return { ...s, sourceUrl: await getSignedFileUrl(sourceKey) };
          } catch (e) {
            console.error("Failed to sign URL for", sourceKey, e);
          }
        }
        return s;
      })
...
export async function getMessages(chatId: string) {
  ...
  try {
    const messages = await db
      ...
      // Chronological order — `id` is a random UUID, so order by createdAt.
